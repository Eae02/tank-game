...
namespace TankGame
{
	    ...
	    : m_min(min), m_max(max), m_snap(snap / (max - min)), m_roundExp(FloatEqual(snap, 0.0f) ? 1.0f : snap)
	{
		...
		{
			s_knobTexture = std::make_unique<Texture2D>(Texture2D::FromFile(resDirectoryPath / "ui" / "slider-knob.png", 4));
			CallOnClose([] { s_knobTexture = nullptr; });
		}
		
		if (s_labelTexture== nullptr)
		{
			s_labelTexture = std::make_unique<Texture2D>(Texture2D::FromFile(resDirectoryPath / "ui" / "slider-label.png", 4));
