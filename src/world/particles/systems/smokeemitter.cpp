...
namespace TankGame
{
	StackObject<Texture2DArray> SmokeEmitter::s_smokeTexture;
	...
	{
		if (s_smokeTexture.IsNull())
		{
			s_smokeTexture.Construct(512, 512, 1, Texture::GetMipmapCount(512), GL_RGBA8);
			s_smokeTexture->LoadLayerFromFile(0, GetResDirectory() / "particles" / "smoke.png");
			s_smokeTexture->SetupMipmapping(true);
			
			CallOnClose([] { s_smokeTexture.Destroy(); });
		}
